#include<stdio.h>
#include <stdlib.h>
//derekursywacja stosem;
int count_routes(int** board, int N, int from_x, int from_y, int to_x, int to_y) {
  if(from_x>=N||from_y>=N) return 0;
  if(board[from_x][from_y]==0)return 0;
  
  if(from_x==to_x&&from_y==to_y) return 1;
  if(from_x>to_x||from_y>to_y) return 0;
  int left = count_routes(board,N,from_x+1,from_y,to_x,to_y);
  int down = count_routes(board,N,from_x,from_y+1,to_x,to_y);
  int diag = count_routes(board,N,from_x+1,from_y+1,to_x,to_y);
